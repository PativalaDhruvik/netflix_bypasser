/**
 * FlixBypass - Early Injection Script (runs at document_start)
 *
 * STRATEGY: Response interception, NOT request blocking.
 * 
 * All GraphQL requests go through normally (title dialogs work).
 * But RESPONSES containing household verification data are intercepted
 * and returned as empty data, so Netflix never triggers the modal.
 *
 * This runs at document_start, before any Netflix JavaScript,
 * ensuring the patch is in place for ALL requests including initial page load.
 */
...
(function () {
    ...
    (function() {
        ...
        window.fetch = function(input, init) {
            ...
            if (url.indexOf(GRAPHQL_HOST + '/graphql') !== -1) {
                ...
                if (typeof reqBody === 'string' && containsHouseholdData(reqBody)) {
                    console.log('[FlixBypass] Blocked household request (body match)');
        ...
        XMLHttpRequest.prototype.send = function(body) {
            if (this._flixUrl && this._flixUrl.indexOf(GRAPHQL_HOST + '/graphql') !== -1) {
                ...
                if (containsHouseholdData(this._flixBody)) {
                    console.log('[FlixBypass] Blocked household XHR (body match)');
        ...
        console.log('[FlixBypass] Response interception active (document.' + document.readyState + ')');
    })();
    `;
