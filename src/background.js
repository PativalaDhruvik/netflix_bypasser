...
async function updateEnabledState() {
    try {
        ...
        console.log(`[State] Extension enabled: ${isExtensionEnabled}`);
...
async function injectContentScript(tabId) {
    ...
    try {
        ...
        if (!msg.includes('already been injected') && !msg.includes('Invalid tab ID') && !msg.includes('Cannot access')) {
            console.error(`[Content Script] Error for tab ${tabId}:`, msg);
        }
    }
}
...
async function checkAllTabs() {
    ...
    } catch (e) { console.error("[checkAllTabs]", e); }
}
...
updateEnabledState();
console.log("FlixBypass background script loaded.");